...
<body>
    ...
    <script src="../javascript/sweetalert.js"></script>
        ...
        // Verificar si algún campo está vacío o tiene un valor no válido
        if (buscar === "" || detalles === "" || isNaN(cantidadEnviada) || isNaN(cantidadRecibida) || cantidadEnviada < 1 || cantidadRecibida < 1 || cantidadRecibida > cantidadEnviada) {
            // Agregar la clase de error a los campos que faltan llenar
            if (buscar === "") {
                document.getElementById("buscar").classList.add("input-error");
            } else {
                document.getElementById("buscar").classList.remove("input-error");
            }
            if (detalles === "") {
                document.getElementById("detalles").classList.add("input-error");
            } else {
                document.getElementById("detalles").classList.remove("input-error");
            }
            if (isNaN(cantidadEnviada) || cantidadEnviada < 1) {
                document.getElementById("cantidadEnviada").classList.add("input-error");
            } else {
                document.getElementById("cantidadEnviada").classList.remove("input-error");
            }
            if (isNaN(cantidadRecibida) || cantidadRecibida < 1 || cantidadRecibida > cantidadEnviada) {
                document.getElementById("cantidadRecibida").classList.add("input-error");
            } else {
                document.getElementById("cantidadRecibida").classList.remove("input-error");
            }
            return; // Detener la ejecución si algún campo está vacío o tiene un valor no válido
        }
...
// Añadir evento para cambiar el borde a rojo si el campo "Seleccionar detalle" está vacío
document.getElementById("detalles").addEventListener("change", function() {
        if (this.value.trim() === "") {
